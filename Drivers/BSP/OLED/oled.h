...
/* OLED SPI模式引脚 定义 */
/* 注意:
 * 1, 这里仅定义了 OLED 4线SPI模式驱动时的 引脚定义
 * 2, OLED_SPI_CS 和  和 OLED_SPI_RS 在并口模式下也要用到
 */

/* RST脚直接和MCU的复位脚共用, 所以不用定义 OLED_SPI_RST 脚了 */
...
#define OLED_SPI_CS_CLK_ENABLE()        do{ __HAL_RCC_GPIOC_CLK_ENABLE(); }while(0)   /* PC口时钟使能 */

#define OLED_SPI_RS_PORT                GPIOC
#define OLED_SPI_RS_PIN                 GPIO_PIN_8
#define OLED_SPI_RS_CLK_ENABLE()        do{ __HAL_RCC_GPIOC_CLK_ENABLE(); }while(0)   /* PC口时钟使能 */
...
/* OLED 并口模式WR, RD引脚 定义 */
/* 注意:
 * 1, 这里仅定义了 并口的 WR 和 RD 两个引脚定义. 8位并口的数据线, 由于引脚太多,就不单独定义了
 * 2, 并口模式下, 也要用到: OLED_SPI_CS 和 OLED_SPI_RS 这两个引脚
 */
#define OLED_WR_PORT                    GPIOC
#define OLED_WR_PIN                     GPIO_PIN_7
#define OLED_WR_CLK_ENABLE()            do{ __HAL_RCC_GPIOC_CLK_ENABLE(); }while(0)   /* PC口时钟使能 */

#define OLED_RD_PORT                    GPIOC
#define OLED_RD_PIN                     GPIO_PIN_6
#define OLED_RD_CLK_ENABLE()            do{ __HAL_RCC_GPIOC_CLK_ENABLE(); }while(0)   /* PC口时钟使能 */
