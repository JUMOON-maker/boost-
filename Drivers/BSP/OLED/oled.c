...
{
    GPIOB->ODR = (GPIOB->ODR & 0XFF00) | (data & 0X00FF);
}
...
void oled_init(void)
{
    ...
    OLED_SPI_CS_CLK_ENABLE();         /* SPI_CS 时钟使能 */
    OLED_SPI_RS_CLK_ENABLE();         /* SPI_RS 时钟使能 */
    OLED_WR_CLK_ENABLE();             /* WR 时钟使能 */
    OLED_RD_CLK_ENABLE();             /* RD 时钟使能 */
    ...
    HAL_GPIO_Init(GPIOB, &gpio_init_struct);                /* PB0 ~ 7 设置,并口总线 */
    /* CS引脚模式设置 */
    ...
    /* WR引脚模式设置 */
    gpio_init_struct.Pin = OLED_WR_PIN;
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP; 
    gpio_init_struct.Pull = GPIO_PULLUP;                   
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_MEDIUM;       
    HAL_GPIO_Init(OLED_WR_PORT, &gpio_init_struct);     
    /* RD引脚模式设置 */
    gpio_init_struct.Pin = OLED_RD_PIN;
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP; 
    gpio_init_struct.Pull = GPIO_PULLUP;                   
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_MEDIUM;       
...
#else               /* 使用4线SPI 串口模式 */
    OLED_SPI_RST_CLK_ENABLE();  /* SPI_RST 时钟使能 */
    OLED_SPI_CS_CLK_ENABLE();   /* SPI_CS 时钟使能 */
    OLED_SPI_RS_CLK_ENABLE();   /* SPI_RS 时钟使能 */
    OLED_SPI_SCLK_CLK_ENABLE(); /* SPI_SCLK 时钟使能 */
